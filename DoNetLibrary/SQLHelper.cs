...
namespace Common
{
    ...
    public class SQLHelper
    {
        ...
        /// <returns></returns>
        public List<string> getIdentityItems(string TableName)
        {
            List<string> list = null;
            DataSet ds = Query("SELECT name FROM sys.columns WHERE object_id=OBJECT_ID('" + TableName + "')  AND is_identity=1");
        ...
        {
            List<string> list = null;
            DataSet ds = Query("EXEC sp_pkeys @table_name='" + TableName + "'");
            if (ds.Tables[0].Rows.Count > 0)
            {
                list = new List<string>();
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    list.Add(ds.Tables[0].Rows[i]["column_name"].ToString());
                }
            }
            return list;
        }
        ...
        {
            List<string> list = null;
            string str = "select name from syscolumns where id=(select id from sysobjects where xtype='U' and name='" + TableName + "')";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    list.Add(row[0].ToString().ToLower());
                }
            }
            return list;
        }
